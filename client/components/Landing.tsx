...
export function Landing({ inviteCode, busy, error, onCreate, onJoin }: LandingProps) {
  ...
  const [name, setName] = useState(stored?.displayName ?? (inviteCode ? 'Mayada' : 'Mahmihoo'));
  ...
  return <main className="landing-shell">
    ...
    {mode !== 'welcome' && <form className="entry-card glass" onSubmit={submit}>
      ...
      <label>Your name<input maxLength={24} value={name} onChange={(event) => setName(event.target.value)} placeholder="Mahmihoo or Mayada" required /></label>
      ...
      {mode === 'join' && <label>Room code<input value={code} onChange={(event) => setCode(event.target.value.toUpperCase())} placeholder="MAHMIHOO-MAYADA-XXXXXX" required /></label>}
