...
export function App() {
  ...
  const copyInvite = () => {
    ...
    void navigator.clipboard.writeText(shareUrl).then(() => setToast('Invite link copied for Mimo ❤️')).catch(() => setToast(`Share this code with Mimo: ${snapshot.code}`));
  };
  return <main className="world-shell">
    <header className="world-header"><a className="brand" href="/" onClick={(event) => { event.preventDefault(); setView('world'); }}><span>♥</span><b>MIMO &amp; HAMO</b><small>OUR LITTLE WORLD</small></a><div className="room-code"><span>PRIVATE ROOM</span><b>{snapshot.code}</b><button onClick={copyInvite}>Copy invite</button></div><div className="header-actions"><button className={view === 'world' ? 'selected' : ''} onClick={() => setView('world')}>✦ World</button><button className={view === 'adventure' ? 'selected' : ''} onClick={() => setView('adventure')}>🌲 Adventure</button><button className="arcade-button" onClick={() => setArcadeOpen(true)}>Our Arcade <span>♥</span></button></div></header>
...
function parseInvite(path: string) { const match = path.match(/^\/join\/(MIMO-HAMO-[A-Z0-9]{6})$/i); return match?.[1].toUpperCase(); }
