...
export function App() {
  ...
  return <main className="world-shell">
    ...
    {view === 'world' ? <section className="world-layout"><div className="game-stage"><div className="world-hud"><div><span className="hud-kicker">LIVE IN</span><strong>Moonlit Valley</strong><small>Private world · {snapshot.players.filter((player) => player.connected).length} hearts online</small></div><div className="location-pills"><span className="active">Home</span><span>Moon Lake</span><span>Memory Garden</span></div><div className="hud-presence"><span className="presence-dot" />{other?.displayName ?? 'Your love'} is here</div></div><Suspense fallback={<Loading />}><WorldGame ref={worldRef} snapshot={snapshot} selfId={selfId} onMove={move} onCollect={(id) => socket.emit('game:heart:collect', id)} onEmote={emote} /></Suspense><GamePrompt snapshot={snapshot} socket={socket} /><div className="world-bottom-hud"><div><span className="hud-kicker">LOVE LEVEL</span><strong>{Math.floor(snapshot.lovePoints / 100) + 1}</strong><small>{snapshot.lovePoints.toLocaleString()} hearts collected</small></div><div className="world-legend"><span><i className="legend-dot gold" />Shared location</span><span><i className="legend-dot rose" />Memory nearby</span></div><button className="hud-action" onClick={() => setArcadeOpen(true)}>Open arcade <span>↗</span></button></div><MobileControls game={worldRef} onEmote={(value) => worldRef.current?.emote(value)} /></div><aside className="world-sidebar"><ProfileHub name={self.displayName} points={snapshot.lovePoints} unlocked={snapshot.unlocked} /><LoveProgress points={snapshot.lovePoints} unlocked={snapshot.unlocked} /><LoveCompanion /><MemoryWall memories={snapshot.memories} /><Achievements points={snapshot.lovePoints} /><Chat socket={socket} messages={snapshot.messages} selfId={selfId} /></aside></section> : <Suspense fallback={<Loading />}><AdventureScene snapshot={snapshot} selfId={selfId} onMove={move} onCrystal={() => socket.emit('adventure:crystal')} onBack={() => setView('world')} /></Suspense>}
